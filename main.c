...
int InitRtpUdpSock(int port, char *ipaddr, struct sockaddr_in *addr)
{
	...
	sendto(sockid, buff, strlen(buff), 0, (struct sockaddr *)&sockaddr, sizeof(sockaddr));
...
//	int len, ret;
	...
	{	
		...
		//GetVideoStream(framebuff, &framelen, &FramePts);//获取一帧数据、长度、时间戳 
                ...
                if(0x62773330 == frameInfo.frame_type)
                        rtp_hdr->payload = 98;
                else
                        rtp_hdr->payload = 96;
		...
		sendbuff[framelen-(1480*i)] = '\0';
		sendnum = sendto(sockclient, sendbuff, framelen-(1480*i), 0,(struct sockaddr*)&sintcpclientaddr, sizeof(sintcpclientaddr));	
